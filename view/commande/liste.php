...
<div class="container" style="margin-top: 90px; ">
    <div class="panel panel-info">
        <div class="panel-heading" align="center">Mes Commandes</div>
        <div class="panel-body">
            <button type="button" style="margin-bottom: 5px;" id="addnewcom" class="btn btn-primary" data-toggle="modal"
            ...
            <table class="ui celled table" id="example" style="width:100%; margin-left: auto;">
                ...
                <tbody>
                <?php
                while($result=mysqli_fetch_row($infosCommandes))
                {
                    $numcom = str_pad($result[0], 5, "0", STR_PAD_LEFT);
                    ...
                    echo "
                    <tr>
                        <td style='text-align: center'>$numcom</td>
                        ...
                        <td><center><a class='btn btn-info btn-xs' href='?page=commande/detail&num=$result[0]'>Détail</a></center></td>
                        <td><center><a class='btn btn-success btn-xs' href='?page=commande/bon&num=$result[0]'>Bon de commande</a></center></td>
                        <td><center><a class='btn btn-danger btn-xs' href='?page=commande/detail&num=$result[0]'>Supprimer</a></center></td>
                    </tr>
                    ";
                }
                ...
                <div class="row">
                    ...
                    <div class="panel-heading" align="center"><h5><b>Article</b></h5></div>
                        <div class="form-group">
                            ...
                            <select class='selectpicker show-menu-arrow form-control' type="text" name="fournisseur" id="fournisseur">
                                <option value="" > <?php echo "Choisir le fournisseur";?> </option>
                                <?php
                            ...
                            <span id="err_fournisseur" class="text-danger"></span>
                        </div>
                        <div class="form-group">
                    ...
                    <form method="post" id="list_articles">
                        ...
                        <div class="panel-heading" align="center"><h5><b>Article(s) sélectionné(s)</b></h5></div>
                            ...
                            <div class="form-group">
                                <input class="btn btn-success" type="submit" name="commander" value="Commander" style="float:right;"/>
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    $(document).ready(function(){
        ...
        $('#addnewcom').click(function () {
            ...
            $('#couleur').val('');
            $('#qte').val('');
            $('#err_fournisseur').text('');
            $('#err_article').text('');
            $('#err_couleur').text('');
            $('#err_qte').text('');
            $('#fournisseur').css('border-color', '');
        ...
        $('#add').click(function () {
            ...
            if($('#fournisseur').val() == ''){
                err_fournisseur = 'Choisir un fournisseur';
                $('#err_fournisseur').text(err_fournisseur);
                $('#fournisseur').css('border-color', '#cc0000');
                fournisseur = '';
            }else {
                err_fournisseur = '';
                $('#err_fournisseur').text(err_fournisseur);
            ...
            if(err_fournisseur != '' || err_article != '' || err_couleur != '' || err_qte != ''){
                return false;
            }else {
                if($('#add').text() == 'Ajouter'){
                    count = count + 1;
                    output = '<tr id="row_'+count+'">';
                    output += '<input type="hidden" name="hidden_fournisseur[]" id="fournisseur'+count+'" value="'+fournisseur+'"/></td>';
        ...
        $('#list_articles').on('submit', function () {
            ...
            if(count_data > 0){
                ...
                $.ajax({
                    url: "<?php echo base_url(); ?>controller/CommandeController.php",
                    method: "POST",
                    data: form_data,
                    success: function(data) {
                        $('#lesarticles').find("tr:gt(0)").remove();
                        $('#action_alert').html('<center><p style="color: green">Commande effectuée avec succes</p></center>');
